...
const escapeMarkup = (text = '') => {
  ...
  escaped = escaped.replace(/(__[a-zA-Z{}]*__)/g, (match) => `pass:[${match}]`);
  return escaped;
};
...
const createLinkTitle = config => schemaProps => {
  //if(!schemaProps.type) console.log(`${schemaProps['$ref']}:${schemaProps.type}`);
  if (schemaProps.type) {
    return schemaProps.type
  }
  return schemaProps['$ref'].split(".").pop()
  return
  const ref = config.refs[schemaProps['$ref'].replace('#/definitions/', '')];
  if(ref)
    return `../${ref.path}/${ref.filename}#${ref.anchor}`;
