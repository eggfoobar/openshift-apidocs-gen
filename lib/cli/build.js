...
function getXRefKeys(obj, results = new Map()) {
  // Hardcoding for now since metadata values use this
  results.set("io.k8s.apimachinery.pkg.apis.meta.v1.ListMeta", true)
  ...
  for(const { definition, operations } of allResources) {
    ...
    const gatherRelatedSchemas = function fn(relatedSchemas, used) {
      relatedSchemas.reduce((accum, schemaId) => {
        ...
        const collectedXRefs = getXRefKeys(fromSchemaId)
        for (const key of collectedXRefs.keys()) {
