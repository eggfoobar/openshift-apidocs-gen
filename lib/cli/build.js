...
const { defaultResourceMap } = require('../constants');
...
// Find any cross reference to add the resource
function getXRefKeys(obj, results = new Map()) {
  if (obj !== null && typeof obj === 'object') {
    for (const key in obj) {
        if (obj.hasOwnProperty(key) && key === "$ref" && typeof obj[key] === 'string') {
          results.set(obj[key].replace('#/definitions/', ''), true);
        }
        getXRefKeys(obj[key], results);
    }
  }
  return results;
}
...
async function action(oapiSpecFile, cmd) {
  ...
  let accumXRefMap = defaultResourceMap;
  ...
  for(const { definition, operations } of allResources) {
    ...
    const gatherRelatedSchemas = function fn(relatedSchemas, used) {
      relatedSchemas.reduce((accum, schemaId) => {
        if (schemaId.startsWith('.v1.')) {
          schemaId = "io.k8s.api.core"+schemaId
        }
        ...
        accumXRefMap = getXRefKeys(fromSchemaId, accumXRefMap)
        for (const key of accumXRefMap.keys()) {
          const fromSchemaId = definitions.getBySchemaId(key);
          if(fromSchemaId) {
            if(!accum.find(v => v == key))  accum.push(key);
          }
        }
